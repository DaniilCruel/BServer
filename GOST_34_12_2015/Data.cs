using System;
using System.IO;

namespace GOST_34_12_2015
{
    class Data
    {
        public static readonly byte[] roundConstants = {
        ,
        };

        public static readonly byte[] pi = {
        ,
        };

        public static readonly byte[] inversedPi = {
        ,
        };

        public static readonly byte[] precomputedLSTable = new byte[16 * 256 * 16];
        public static readonly byte[] precomputedInversedLSTable = new byte[16 * 256 * 16];

        //считывание предвычислительной таблицы
        public static void getPrecomputedLSTable()
        {
            string[] text = File.ReadAllLines(Directory.GetCurrentDirectory() + "\\precomputedLSTable");
            string[] subStr;
            char[] trim = { ',' };
            for (int i = 0; i < 16 * 256; i++)
            {
                subStr = text[i].Split(trim, 17);
                for (int j = 0; j < 16; j++)
                {
                    precomputedLSTable[i * 16 + j] = Convert.ToByte(subStr[j], 16);
                }
            }
        }

        //считывание обратной предвычислительной таблицы
        public static void getPrecomputedInversedLSTable()
        {
            string[] text = File.ReadAllLines(Directory.GetCurrentDirectory() + "\\precomputedInversedLSTable");
            string[] subStr;
            char[] trim = { ',' };
            for (int i = 0; i < 16 * 256; i++)
            {
                subStr = text[i].Split(trim, 17);
                for (int j = 0; j < 16; j++)
                {
                    precomputedInversedLSTable[i * 16 + j] = Convert.ToByte(subStr[j], 16);
                }
            }
        }

    }
}
